...
//            array_push( $arr_whereCondition, ['item.name', 'like', '%'. $keyValue[1] .'%'] );
            array_push( $arr_whereCondition, [\DB::raw('BINARY `name`'), 'like', '%'. $keyValue[1] .'%'] );
//            array_push( $arr_whereCondition, [\DB::raw('BINARY `name`'), 'like', \DB::raw('BINARY `%'.$keyValue[1].'%`')] );
//            array_push( $arr_whereCondition, ['item.name', 'ilike ', '%'. $keyValue[1] .'%'] );
//            Invite::whereRaw("BINARY `token`='{$token}'")->first()
//            Invite::where(DB::raw('BINARY `token`'), $token)->first();
        }

        $itemQuery -> where( $arr_whereCondition );
        $itemQuery -> orderBy( 'item.id', 'desc' );
...
//        return $result;
        return redirect('item/edit/'. $insertedId );
    }
