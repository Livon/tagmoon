...
class TagRecentlyUsedCtrl extends Controller
{
    ...
    public function recentlyUsed(){
        ...
        $tags = DB::table('tag_recently_used')
     ...
     * @return Json
     */
    public static function logging( $tagName ){


        $tagId = TagCtrl::getId_by_TagName( $tagName );

        $insertedId = 0 ;

        if( $tagId != 0 ){
            $insertedId = \DB::table('tag_recently_used')->insertGetId(
                ['tagId' => $tagId ]
            );
        };
...
//            $subQuery = DB::table('abc')->where('col1', 'xxx')->where('col2', xxx)->groupBy('col1');
        ...
        $subQuery = DB::table('tag_recently_used')
            -> select( 'tagId', 'timestamp' )
            -> orderBy('timestamp', 'desc');

//                SELECT DISTINCT a.tagId
//                FROM ( SELECT tru.tagId
//                FROM tag_recently_used tru
//                ORDER BY tru.timestamp DESC) a;


        $query = DB::table(DB::raw("({$subQuery->toSql()}) as sub"))
//            -> select(DB::raw('sub.tagId','tag.tagName'))
            -> select( 'sub.tagId','tag.tagName')
            -> leftJoin('tag', 'sub.tagId', 'tag.id' )
            -> orderBy('timestamp', 'desc')
            -> distinct();
//            -> paginate(20);
...
//        $tags = DB::table('tag_recently_used')
//            -> select( 'tag_recently_used.tagId', 'tag.tagName')
//            -> leftJoin('tag', 'tag_recently_used.tagId', 'tag.id' )
//            -> orderBy('timestamp', 'desc')
//            -> distinct()
//            -> paginate(20);
